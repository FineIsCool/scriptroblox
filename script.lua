...
local EmoteTab = Window:CreateTab("Emote", "person-standing")
...
local function zeezyFrontflip()
    ...
    for i = 1, 360 do 
        delay(i/720, function()
            Humanoid.Sit = true
            HumanoidRootPart.CFrame = HumanoidRootPart.CFrame * CFrame.Angles(-h, 0, 0)
        end)
    end
    wait(0.55)
    Humanoid.Sit = false
...
EmoteTab:CreateToggle({
    ...
    Callback = function(value)
        ...
        Rayfield:Notify({
            Title = "feFlip",
            Content = "feFlip " .. (isEnabled and "enabled" or "disabled") .. ".",
            Duration = 3,
            Image = "user"
        })
    end
...
-- Keybinds
getgenv().FrontflipKeybind = EmoteTab:CreateKeybind({
    Name = "Frontflip Keybind",
    CurrentKeybind = "Z",
    Flag = "FrontflipKeybind",
    Callback = function(newKey)
        if typeof(newKey) == "string" then
            local foundKey = Enum.KeyCode[newKey:upper()]
            if foundKey then
                FrontflipKey = foundKey
                Rayfield:Notify({
                    Title = "Keybind Updated",
                    Content = "Frontflip keybind set to " .. newKey .. ".",
                    Duration = 3,
                    Image = "user"
                })
            else
                Rayfield:Notify({
                    Title = "Keybind Error",
                    Content = "Invalid keybind: " .. newKey,
                    Duration = 3,
                    Image = "user"
                })
            end
...
getgenv().BackflipKeybind = EmoteTab:CreateKeybind({
    ...
    Callback = function(newKey)
        if typeof(newKey) == "string" then
            local foundKey = Enum.KeyCode[newKey:upper()]
            if foundKey then
                BackflipKey = foundKey
                Rayfield:Notify({
                    Title = "Keybind Updated",
                    Content = "Backflip keybind set to " .. newKey .. ".",
                    Duration = 3,
                    Image = "user"
                })
            else
...
getgenv().AirjumpKeybind = EmoteTab:CreateKeybind({
    Name = "Airjump Keybind",
    CurrentKeybind = "C",
    Flag = "AirjumpKeybind",
    Callback = function(newKey)
        if typeof(newKey) == "string" then
            ...
            if foundKey then
                ...
                Rayfield:Notify({
                    Title = "Keybind Updated",
                    Content = "Airjump keybind set to " .. newKey .. ".",
                    Duration = 3,
                    Image = "user"
                })
            else
...
-- Bind actions using UserInputService
local UserInputService = game:GetService("UserInputService")
local frontflipConn, backflipConn, airjumpConn

local function setupBinds()
    if frontflipConn then frontflipConn:Disconnect() end
    if backflipConn then backflipConn:Disconnect() end
    if airjumpConn then airjumpConn:Disconnect() end

    frontflipConn = UserInputService.InputBegan:Connect(function(input, gameProcessed)
        if not gameProcessed and input.KeyCode == FrontflipKey then
            zeezyFrontflip()
        end
    end)

    backflipConn = UserInputService.InputBegan:Connect(function(input, gameProcessed)
        if not gameProcessed and input.KeyCode == BackflipKey then
            zeezyBackflip()
        end
    end)

    airjumpConn = UserInputService.InputBegan:Connect(function(input, gameProcessed)
        if not gameProcessed and input.KeyCode == AirjumpKey then
            zeezyAirjump()
        end
    end)
end
...
-- Update binds when keybinds change (using Callback instead of OnChanged)
getgenv().FrontflipKeybind.Callback = function(newKey)
    if typeof(newKey) == "string" then
        local foundKey = Enum.KeyCode[newKey:upper()]
        if foundKey then
            FrontflipKey = foundKey
            setupBinds() -- Rebind after key change
            Rayfield:Notify({
                Title = "Keybind Updated",
                Content = "Frontflip keybind set to " .. newKey .. ".",
                Duration = 3,
...
getgenv().BackflipKeybind.Callback = function(newKey)
    if typeof(newKey) == "string" then
        local foundKey = Enum.KeyCode[newKey:upper()]
        if foundKey then
            BackflipKey = foundKey
...
getgenv().AirjumpKeybind.Callback = function(newKey)
    if typeof(newKey) == "string" then
        local foundKey = Enum.KeyCode[newKey:upper()]
        if foundKey then
            AirjumpKey = foundKey
...
local Section = EmoteTab:CreateSection("Jerk Off")
...
-- Initialize variables
local time_pos_jerking = 0.6
local jerking_off_speed = 1
local jerking_enabled = false
local jerkAnim = nil
local jerkTrack = nil
wait(0.1)

-- Slider: Jerk Off Time Position
getgenv().JerkOffTimePos = EmoteTab:CreateSlider({
    ...
    Callback = function(updateTimePosJerk)
        ...
        Rayfield:Notify({
            Title = "Jerk Off Time Position",
            Content = "Time position set to " .. updateTimePosJerk .. ".",
            Duration = 3,
            Image = "user"
        })
    end
...
-- Slider: Jerk Off Speed
getgenv().Jerking_Speed = EmoteTab:CreateSlider({
    Name = "Jerk Off Speed",
    Range = {0.2, 3},
    Increment = 0.1,
    Suffix = "Speed",
    ...
    Callback = function(jerk_speed)
        ...
        Rayfield:Notify({
            Title = "Jerk Off Speed",
            Content = "Speed set to " .. jerk_speed .. ".",
            Duration = 3,
            Image = "user"
        })
    end
...
-- Toggle: Jerk Off
getgenv().JerkOffTool = EmoteTab:CreateToggle({
    Name = "Jerk Off",
    CurrentValue = false,
    Flag = "JerkingOffBruh",
    Callback = function(getJerkingScript)
        jerking_enabled = getJerkingScript
        if getJerkingScript then
            if jerkTrack then
                Rayfield:Notify({
                    Title = "Failure",
                    Content = "Jerk Off Animation already loaded.",
                    Duration = 5,
                    Image = "user"
                })
                return
            ...
            task.spawn(function()
                while jerking_enabled do
                    task.wait()
                    if not jerking_enabled then return end
                    jerkTrack:Play()
                    jerkTrack:AdjustSpeed(jerking_off_speed)
                    jerkTrack.TimePosition = time_pos_jerking
                    task.wait(0.1)

                    while jerkTrack.TimePosition < time_pos_jerking do
                        task.wait(0.1)
                    end
            ...
            Rayfield:Notify({
                Title = "Jerk Off",
                Content = "Jerk Off Animation enabled.",
                Duration = 3,
                Image = "user"
            })
        else
            jerking_enabled = false
            wait(0.1)
            if jerkAnim and jerkTrack then
                jerkTrack:Stop()
                jerkAnim:Destroy()
                jerkTrack:Destroy()
                jerkAnim = nil
                ...
                Rayfield:Notify({
                    Title = "Failure",
                    Content = "Jerk Off Animation is not loaded!",
                    Duration = 5,
                    Image = "user"
                })
            end
            Rayfield:Notify({
                Title = "Jerk Off",
                Content = "Jerk Off Animation disabled.",
                Duration = 3,
                Image = "user"
            })
        end
